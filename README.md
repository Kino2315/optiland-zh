...
会在桌面放一个「Optiland 中文版」图标——用的是 Optiland 自己的应用图标，
双击直接以中文界面启动。

| 想要什么 | 加什么参数 |
|---|---|
| 放到别的目录 | `--shortcut-dir <目录>` |
| 带控制台的版本（想看到报错） | `--console-shortcut` |
| 删掉它 | `optiland-zh --uninstall-shortcut` |

几个说明：

- **pip 不会自动建桌面图标**，这是有意的。装个包就去动用户的桌面，
  在 CI 和无桌面环境里还会失败得莫名其妙。所以这一步做成显式命令。
- 图标文件生成在 `%LOCALAPPDATA%\optiland-zh\`，**没有塞进任何仓库**，也没放桌面。
- `--uninstall-shortcut` **只删自己建的那一个名字**，不会去动你手工做的快捷方式。
