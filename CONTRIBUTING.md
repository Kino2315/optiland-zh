...
界面上还有英文，**先查那句话在不在词库里** —— 这一步就分出了该做什么：

```sh
optiland-zh --lookup "Save System"
```

它内部会把 `&` 和结尾的省略号忽略掉，所以你**照着屏幕上看到的原样打**就行
（屏幕上可能是 `Save System`，词库里的键是 `&Save System`）。

- **查得到** → 词库里有译文，界面上却还是英文 → **补丁没拦住那个控件**，是引擎的问题。
- **查不到** → 还没人翻 → 那是[补词条](#一补词条不需要懂-python)的事，不是引擎的问题。

确认是引擎的问题之后，再看它属于哪类拦截点：
