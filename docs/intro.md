...
**为什么要分档？** 单看"96.8%"没法判断里面有多少水份。一个不可拆解的数字不算证据。

分档的判据是**「还剩没剩英文」**，不是「能不能在词库里找到」：

- `exact-value`（974）—— 文字与词库某条译文逐字符相等，最硬。
- `cjk-only`（30）—— **一个拉丁字母都没有**。原界面是英文，渲染出来全是中文，
  那就是译好了。**这一档是确定的，不是启发式**，没必要打折。
- `cjk-mixed`（43）—— 中英混合，还剩拉丁字母。实测全是 Qt 自动派生的快捷键字母
  （`文件(&F)` → `文件(F)`）或本来就不该翻的专有名词（`从 CODE V 导入(C)`）。
